...
const regController=async(req,res)=>{
    ...
    if (!req.body) {
      return res.status(StatusCodes.BAD_REQUEST).json({ msg: "Request body is missing" });
    }
    ...
    if (!name || !email || !password || !mobile) {
      return res.status(StatusCodes.BAD_REQUEST).json({ msg: "All fields (name, email, password, mobile) are required" });
    }
